...
enum SoundLibraryError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingBundledAudio:
            "No bundled audio files were found"
        case let .emptyFolder(path):
...
struct SoundLibrary {
    ...
    private func matchesMode(_ fileURL: URL, mode: SoundMode) -> Bool {
        ...
        case .halo:
            return isHalo
        case .volchara:
            return !isPain && !isSexy && !isHalo
        }
    }

    private func bundledAudioFiles(subdirectory: String?) -> [URL] {
        let all = supportedExtensions
            .flatMap { Bundle.main.urls(forResourcesWithExtension: $0, subdirectory: subdirectory) ?? [] }
